...
export default function ScribePage(props: { isDemo?: boolean, onDemoComplete?: () => void, onDemoStart?: () => void, demoTrigger?: number }) {
    ...
    const { patients, transcripts, setTranscripts, activePatientId, setActivePatientId, addTranscriptRecord, updatePatient, pharmacyAlerts, setPatients } = useGlobalState();
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (SpeechRec) {
                ...
                recognition.lang = 'en-IN'; // Indian English — transcribes Hinglish in Roman script
                ...
                recognition.onresult = (event: any) => {
                    ...
                    if (finalTranscriptStr) {
                        ...
                        if (currentId) {
                            addTranscriptRecord(currentId, { speaker: 'Doctor/Patient (Live)', text: finalTranscriptStr.trim(), time: timeStr });
                        }
    ...
    // Real-Time Summarizer Flow (Gemini debounced execution)
    useEffect(() => {
        if (!activePatientId) return;
        const currentTranscript = transcripts[activePatientId];
        if (!currentTranscript || currentTranscript.length === 0) return;

        // Debounce: Trigger summary 2.5 seconds after doctor or patient stops talking
        const timer = setTimeout(async () => {
            setIsSummarizing(true);
            try {
                const dialogueText = currentTranscript
                    .map((t: any) => `${t.speaker || 'Doctor/Patient'}: ${t.text}`)
                    .join('\n');

                const response = await fetch('/api/summarize', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ transcript: dialogueText })
                });

                if (response.ok) {
                    const data = await response.json();
                    if (data && !data.error) {
                        updatePatient(activePatientId, {
                            hpi: data.complaint || '',
                            diagnosis: data.diagnosis || '',
                            safetyFlag: data.safetyFlags || null,
                            assessment: [
                ...
                console.error("Real-time summarizer failed:", e);
            } finally {
    ...
    useEffect(() => {
        ...
        if (emrContainerRef.current) {
            setTimeout(() => {
                ...
                let targetTop = 0;
                if (activePatient?.safetyFlag || (activePatient?.assessment && activePatient.assessment.length > 0)) {
                    targetTop = emrContainerRef.current.scrollHeight;
                } else if (activePatient?.diagnosis) {
                    targetTop = emrContainerRef.current.scrollHeight * 0.4;
                } else {
                    targetTop = 0;
                }
        ...
        <div className={styles.dashboardContent} style={{ padding: '24px 32px' }}>
            ...
            <div className={styles.workflowCard} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', marginBottom: '8px', borderLeft: isRecording ? '4px solid #ef4444' : '4px solid var(--color-accent-green)', overflow: 'visible' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    {isSummarizing && (
                        <div style={{ fontSize: '12px', color: 'var(--color-accent-green)', animation: `${styles.pulse} 1.5s infinite` }}>
                    ...
                    {!isDemo && transcripts[activePatientId]?.length > 0 && (
                        <button
                            onClick={() => {
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(350px, 400px) 1fr', gap: '24px', flex: 1, height: 'calc(100vh - 160px)' }}>
                ...
                <div ref={emrContainerRef} style={{ display: 'flex', flexDirection: 'column', gap: '16px', height: '100%', overflowY: 'auto', paddingRight: '8px' }}>
                    ...
                    <div className={styles.workflowCard} style={{
                        ...
                        border: activePatient?.hpi ? '1px solid rgba(16, 185, 129, 0.4)' : '',
                        boxShadow: activePatient?.hpi ? '0 0 20px rgba(16, 185, 129, 0.15)' : ''
                    }}>
                        ...
                        <p style={{ color: activePatient?.hpi ? 'var(--color-text-primary)' : 'var(--color-text-secondary)', fontSize: '14px', lineHeight: 1.5, margin: 0 }}>
                            {activePatient?.hpi || 'AI is listening for symptoms and history...'}
                        </p>
                    </div>
                    ...
                    <div className={styles.workflowCard} style={{
                        padding: '20px',
                        transition: 'all 0.5s ease',
                        border: activePatient?.diagnosis ? '1px solid rgba(16, 185, 129, 0.4)' : '',
                        boxShadow: activePatient?.diagnosis ? '0 0 20px rgba(16, 185, 129, 0.15)' : ''
                    }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                        ...
                        <div style={{ background: 'rgba(255,255,255,0.03)', padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            {activePatient?.diagnosis ? (
                                <span style={{ color: 'var(--color-text-primary)', fontWeight: 600, fontSize: '14px' }}>{activePatient.diagnosis}</span>
                            ) : (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                        <div className={styles.workflowCard} style={{
                            padding: '20px',
                            transition: 'all 0.5s ease',
                            border: activePatient?.safetyFlag ? '1px solid rgba(239, 68, 68, 0.4)' : '',
                            boxShadow: activePatient?.safetyFlag ? '0 0 20px rgba(239, 68, 68, 0.15)' : ''
                        }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                                <h4 style={{ margin: 0, color: 'var(--color-text-primary)', fontSize: '15px' }}>Clinical Safety Check</h4>
                            </div>
                            {activePatient?.safetyFlag ? (
                        ...
                        <div className={styles.workflowCard} style={{
                            padding: '20px',
                            transition: 'all 0.5s ease',
                            border: (activePatient?.assessment && activePatient.assessment.length > 0) ? '1px solid rgba(16, 185, 129, 0.4)' : '',
                            boxShadow: (activePatient?.assessment && activePatient.assessment.length > 0) ? '0 0 20px rgba(16, 185, 129, 0.15)' : ''
                        }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                                <h4 style={{ margin: 0, color: 'var(--color-text-primary)', fontSize: '15px' }}>Medications (Rx) &amp; Plan</h4>
                            </div>
                            <div style={{ color: (activePatient?.assessment && activePatient.assessment.length > 0) ? 'var(--color-text-primary)' : 'var(--color-text-secondary)', fontSize: '14px', lineHeight: 1.6 }}>
                                {activePatient?.assessment && activePatient.assessment.length > 0 ? (
                                    <ul style={{ margin: 0, paddingLeft: '16px' }}>
                                        {activePatient.assessment.map((item: string, i: number) => (
                                            <li key={i}>{item}</li>
                                        ))}
                                    </ul>
                                ) : (
                                    <span style={{ fontStyle: 'italic' }}>Listening for prescriptions...</span>
                                )}
                            </div>
                        </div>
                    </div>
